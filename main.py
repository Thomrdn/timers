...
def index():
  ...
  shrine_reset_date = datetime.fromisoformat(shrine_reset_date_str).astimezone(
    timezone.utc) + timedelta(hours=2)
  rift_end_date = datetime.fromisoformat(rift_end_date_str).astimezone(
    timezone.utc) + timedelta(hours=2)
  grade_reset_date_str = data["data"]["timers"]["gradeResetDate"]
  grade_reset_date = parser.parse(grade_reset_date_str)

  # Calcul des durées restantes avec fuseau horaire
  shrine_reset_delta = shrine_reset_date - datetime.now(
    tz=shrine_reset_date.tzinfo)
  rift_end_delta = rift_end_date - datetime.now(tz=rift_end_date.tzinfo)
  grade_reset_delta = grade_reset_date - datetime.now(
    tz=grade_reset_date.tzinfo)
  ...
  result = "Nombre de joueurs Steam en ligne : {} | Raffraichissement du Temple : {} jours, {} heures, {} minutes, {} secondes. | Fermeture de la Faille : {} jours, {} heures, {} minutes, {} secondes. | Réinitialisation des Rangs : {} jours, {} heures, {} minutes, {} secondes. |".format(
    player_count, shrine_reset_delta.days, shrine_reset_delta.seconds // 3600,
    (shrine_reset_delta.seconds // 60) % 60, shrine_reset_delta.seconds % 60,
    rift_end_delta.days, rift_end_delta.seconds // 3600,
    (rift_end_delta.seconds // 60) % 60, rift_end_delta.seconds % 60,
    grade_reset_delta.days, grade_reset_delta.seconds // 3600,
    (grade_reset_delta.seconds // 60) % 60, grade_reset_delta.seconds % 60)
